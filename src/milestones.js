...
const milestones = (container, width, height) => {
	const g = container
		.append('g')
	...
	data.asyncItems().then(raw => {
		...
		const jqueryData = raw[0]
			.map(item => ({ ...item, position: 1 }))
			.filter(release => /^\d+\.\d+\.\d+$/.test(release.version));
		const d3Data = raw[1].map(item => ({ ...item, position: 3 }));
		const angularData = raw[2]
			.map(item => ({ ...item, position: 2 }))
			.filter(release => /^\d+\.\d+\.\d+$/.test(release.version));
		const polymerData = raw[3]
			.map(item => ({ ...item, position: 4 }))
			.filter(release => release.version.slice(0, 6) !== 'v0.0.2');
		const reactData = raw[4].map(item => ({ ...item, position: 5 }));
		const vuejsData = raw[5].map(item => ({ ...item, position: 6 }));
