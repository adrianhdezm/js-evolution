...
// Mapping RawData to Projects
    ...
    data.sort((a, b) => parseTime(a.minRelease.date) - parseTime(b.minRelease.date));
...
//adding marks
    ...
    let projects = g.selectAll('g.version')
        .data(data)
        .enter().append('g');
    ...
    projects
        .attr('class', 'project')
        .attr('transform', d => `translate(${ timeScale(parseTime(d.minRelease.date))},${axeVerticalPosition + d.offset})`);
    ...
    projectRect
        .attr('x', 0)
        .attr('y', 0)
        .attr('width', d => timeScale(parseTime(d.maxRelease.date)) - timeScale(parseTime(d.minRelease.date)))
        .attr('height', 20)
    ...
    ProjectMaxVersionText
        .attr('x',d => timeScale(parseTime(d.maxRelease.date)) - timeScale(parseTime(d.minRelease.date)))
        .attr('y', 15)
